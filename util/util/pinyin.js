...
function fTranslate(sChars, sSplitter) {
    ...
    for (var i = 0, l = sChars.length; i < l; i++) {
        sChar = sChars.charAt(i);
        sPinyin = that.getCharPy(sChar);
        ...
        if (sPinyin) {
            ...
            aResult.push(sPinyin);
        } else {
            if (/[a-z|A-Z|\d]/g.test(sChar)) {
                sPre += sChar;
            } else if (sPre !== '') {
                aResult.push(sPre);
                sPre = '';
    ...
    sPinyin = aResult.join(sSplitter).toLowerCase();
    aResult.length = 0;
    return sPinyin;
}

function fGetCharPy(sChar) {
    var aItem;
    ...
    if (!nDictLength) {
        nDictLength = aDict.length;
    }

    for (var i = 0, l = nDictLength; i < l; i++) {
        aItem = aDict[i];
        if (aItem[1].indexOf(sChar) !== -1) {
            return aItem[0];
        }
    }
