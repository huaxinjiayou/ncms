...
module.exports = {
    route: '/profile/:uid/collects',
    filter: ['checkuser', 'profiledata'],
    get: eval(windAsync(fGetProfileCollects))
};

function fGetProfileCollects(req, res, next) {
    var that = this;
    ...
    var oCollectData = $await(book.page(oFilter));
    var aCovers = $await(file.findCover(oCollectData.data));

    // 其他信息
    var aIds = oCollectData.data.map(function (oItem) {return oItem.id;});
