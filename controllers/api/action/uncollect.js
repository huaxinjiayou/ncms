...
module.exports = {
    filter: ['login'],
    post: eval(windAsync(fPostActionUnCollect))
}

function fPostActionUnCollect(req, res, next) {
    var that = this;
    ...
    $await(action.cancel(that.user, sObjType, sObjId, action.actionType.collect));
    that.json();
}